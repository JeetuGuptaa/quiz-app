...
export default function AddQuestion(props) {
  ...
  const handleSubmit = async (data) => {
    const response = await fetch(
      "https://quiz-app-server-azure.vercel.app/api/v1/quiz/createQuiz/",
      {
        method: "POST",
        headers: {
    ...
    return (
      <div id="cur-question-container">
        ...
        {options.flags.question && options.question === "" ? (
          <small className="warning">The question must not be empty.</small>
        ) : (
          ""
        )}
        ...
        {options.flags.selected && options.selected === "" ? (
          <small className="warning">One option must be marked correct</small>
        ) : (
          ""
        )}
